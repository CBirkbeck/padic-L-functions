...
The *full* statement is blocked on the §13/IMC-deferred inverse-limit `Λ(𝒢)`-module
structure: `𝒞_{∞,1}` is defined as the inverse limit of the *topological closures*
`𝒞_{n,1} = clos(𝒟_{n,1}) ⊓ 𝒰_{n,1}` (`cycloClosureOne`), so passing the level-`n` group
generation `cycloUnitsPlus_eq_closure_gammas` (which generates the *global* units `𝒟_n^+`,
not their closures) up to the tower requires either continuity of `Col` (not available —
`Col` is built from the Coleman-series limit construction) or the inverse-limit module
structure of `cycloTower1Plus_cyclic_generator` (whose full form, TeX 3573–3578, is the
deferred §13 input — its current project form has a vacuous second conjunct). All other
ingredients of both milestones (`Col` hom property, the descent through `QuotientGroup.lift`,
the plus-part transport `plusEquiv`/`projPlus`/`isCompl_plusPart_minusPart`, the
`ℤ_p(1)^{⟨c⟩}=0` collapse, surjectivity via `range_Col_eq_ker_chiMoment`) are proved below
without further `sorry`. -/
theorem col_mem_zetaIdeal_iff_mem_cycloTower1 (hp2 : p ≠ 2) {u : NormCompatUnits p}
    (hu : u ∈ unitsTower1 p) :
    Col p u ∈ PadicMeasure.zetaIdeal p hp2 ↔ u ∈ cycloTower1 p := sorry
